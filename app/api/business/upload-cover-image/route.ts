...
export async function POST(request: NextRequest) {
  try {
    ...
    const filePath = `${user.id}/${businessId}-cover-${Date.now()}.${ext}`;
    ...
    if (uploadError) {
      ...
      if (lower.includes('bucket') || lower.includes('not found')) {
        return NextResponse.json(
          {
            error:
              'SETUP NEEDED: Create a public Storage bucket named "business-covers" in Supabase Storage.',
          },
          { status: 500 }
        );
      }
    ...
    const { data: publicUrlData } = adminSupabase.storage.from('business-covers').getPublicUrl(filePath);
    ...
    if (updateError) {
      return NextResponse.json({ error: 'Failed to save cover image URL' }, { status: 500 });
    }
